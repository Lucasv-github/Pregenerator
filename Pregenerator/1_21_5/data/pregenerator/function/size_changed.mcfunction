...
tellraw @s ["",{text:"You are about to generate the world: ",bold:true,color:"gold"},{score:{name:"Temp",objective:"pregen_size"},bold:true,color:"dark_red"},{text:" blocks in every direction, do you want to continue?",bold:true,color:"gold"},{text:" Click to start",bold:true,color:"dark_green",click_event:{action:"run_command",command:"/trigger pregen_start set 1"}}]
